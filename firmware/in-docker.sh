...
if [[ -n "${ARGB_ENDPOINT_COUNT:-}" ]]; then
    DOCKER_ARGS+=(-e ARGB_ENDPOINT_COUNT)
fi

if [[ -n "${ARGB_ENDPOINT_LED_COUNT:-}" ]]; then
    DOCKER_ARGS+=(-e ARGB_ENDPOINT_LED_COUNT)
fi
...
if [[ "${CMD[0]:-}" == "idf.py" ]]; then
    IDF_DEFINES=(
        ...
        "-DARGB_ENDPOINT_COUNT=${ARGB_ENDPOINT_COUNT:-1}"
        "-DARGB_ENDPOINT_LED_COUNT=${ARGB_ENDPOINT_LED_COUNT:-12}"
